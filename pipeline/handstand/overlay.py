...
def load_panel(label: str, parquet_path: str | pathlib.Path) -> PanelKeypoints:
    ...
    flag to filter on rather than the coordinates.
    """
...
def render_overlay(
    ...
    and the extra line :data:`NO_POSE_LABEL`.
            ...
            try:
                for packet in video:
                    ...
                    for index, (panel, frame_keys) in enumerate(
                        ...
                        tiles.append(draw_caption(tile, lines))
...
def keypoints_root(data: str | pathlib.Path | None = None) -> pathlib.Path:
    """``<data_dir>/keypoints/mediapipe``, where the parquets live."""
    root = pathlib.Path(data) if data is not None else data_dir()
    return root / "keypoints" / "mediapipe"
...
def panel_parquet_path(
    clip_id: str, mode: str, data: str | pathlib.Path | None = None
) -> pathlib.Path:
    """``<data_dir>/keypoints/mediapipe/<mode>/<clip_id>.parquet``."""
    return keypoints_root(data) / mode / f"{clip_id}.parquet"
...
def missing_parquet_message(
    ...
    command = f"cd pipeline && uv run python -m handstand.pose_mediapipe --rotate {mode}"
...
def require_parquet(
    ...
    path = panel_parquet_path(clip_id, mode, data)
    if not path.is_file():
        raise FileNotFoundError(missing_parquet_message(clip_id, mode, path, video_path))
    return path


def overlay_filename(clip_id: str, labels: Sequence[str]) -> str:
...
def render_clip(
    ...
    *,
    ...
    ``<mode>`` parquets under ``<data_dir>/keypoints/mediapipe/`` and the output
    ...
    panels = [(mode, require_parquet(clip_id, mode, data, video_path)) for mode in modes]
    out_path = overlays_dir(data) / overlay_filename(clip_id, list(modes))
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print(
        f"clip  {report.video_path.name} id={args.clip} "
